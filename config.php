...
function sendMessage($txt, $key = null, $parse ="MarkDown", $ci= null, $msg = null){
    ...
    if(!empty($parse)) $params['parse_mode'] = $parse;
    if(!empty($msg)) $params['reply_to_message_id'] = $msg;
...
// Updated Telegram server IP ranges (2024-2025)
$range = [
        '149.154.160.0/20',  // Expanded range covering 160-175
        '91.108.4.0/22',
        '91.108.8.0/22',
        '91.108.12.0/22',
        '91.108.16.0/22',
        '91.108.56.0/22',
        '91.105.192.0/23',   // Added new Telegram range
        '185.76.151.0/24',   // Added new Telegram range
        '95.161.64.0/20',
    ];
...
function curl_get_file_contents($URL){
    ...
    curl_setopt($c, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; XUI-Bot/10.4)');
...
// Only check membership if a lock channel is configured
if(!empty($channelLock) && $channelLock !== ' '){
    $joniedState = bot('getChatMember', ['chat_id' => $channelLock, 'user_id' => $from_id])->result->status ?? 'member';
} else {
    $joniedState = 'member';
}
...
if ($update->message->document->file_id) {
    ...
    $filetype = 'photo';
    // Always get the highest-resolution photo (last element = largest size)
