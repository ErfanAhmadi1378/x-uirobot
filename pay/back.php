...
function showForm($msg, $type = "", $state = false){
    $isSuccess = ($state === true || strtolower($type) === 'success');
    $icon      = $isSuccess ? '✅' : '❌';
    $color     = $isSuccess ? '#10b981' : '#ef4444';
    $bgColor   = $isSuccess ? '#d1fae5' : '#fee2e2';
    $border    = $isSuccess ? '#6ee7b7' : '#fca5a5';
    $title     = $type ?: ($isSuccess ? 'پرداخت موفق' : 'خطا در پرداخت');
?>
<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    .msg-title { font-size:1.2rem; font-weight:700; color:<?php echo $color; ?>; margin-bottom:12px; }
    .msg-text {
      background:<?php echo $bgColor; ?>; border:1px solid <?php echo $border; ?>;
      border-radius:10px; padding:14px 18px; color:<?php echo $color; ?>;
      font-size:.95rem; font-weight:600;
    }
    .bot-link {
      display:inline-block; margin-top:20px; padding:11px 28px;
      background:linear-gradient(90deg,#4361ee,#7b2ff7); color:#fff;
...
<body>
  <div class="msg-card">
    ...
    <div class="msg-title"><?php echo htmlspecialchars($title); ?></div>
